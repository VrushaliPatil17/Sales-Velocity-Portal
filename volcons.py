    ...
    def add_person_linkedin(self, name, email, job_type, company):
        with self._driver.session() as session:
            ...
            session.run("MERGE(emp:Person {first_name: $name, email: $email, job_type: $job_type,employee:$emp})"
                          "MERGE (com: Company {name: $company, offering: $offering})"
                          "MERGE (emp)-[:WORKS_FOR]->(com)",
                          name=name, email = email, job_type = job_type, company = company,emp=emp, offering = offering)
    @staticmethod
    def match_person_nodes(tx):
        return list(tx.run("MATCH (a:Person) RETURN a.name AS name"))
    
    def add_person_crm(self,customer,employee,company,touch_point,short_term,medium_term,long_term, offering):
        with self._driver.session() as session:
            empY="yes"
            empN="no"
            ...
            session.run("MERGE (empYes:Person{first_name: $employee, email: $email, job_type: $job_type,employee:$empY})"
                        ...
                        "MERGE (comYes: Company {name: $compY, offering: $offering})"
                        "MERGE (empYes)-[:WORKS_FOR]->(compYes)"
                        "MERGE (compNo:Company {name: $compN, offering: $offering})"
                        ...
                        email=email,job_type=job_type,s_term=short_term,m_term=medium_term,l_term=long_term, offering=offering)
    
    def add_person_meta(self, role, offer):
        with self._driver.session() as session:
            for record in session.run("MATCH (emp:Person)"
                                 "WHERE emp.job_type = {role} "
                                 "RETURN emp", role=role):
                session.run("MERGE(off:Offer{role:$role,offering:$offer})"
                            "MERGE(emp)-[:PROVIDES]->(off)",role = role, offer = offer)
                
#    def query(self,role,offer):
#        with self._driver.session() as session:
#            session.run("MATCH ",
...
with open('CRM Data.csv') as csvfile:
    ...
    for row in readCSV:
        print(row)
        book.add_person_crm(row[0],row[1],row[2],row[3],row[4],row[5],row[6], row[7])
